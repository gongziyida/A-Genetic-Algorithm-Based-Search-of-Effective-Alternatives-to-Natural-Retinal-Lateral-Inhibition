...
//    double decay;       // Decay of weight w.r.t. distance
    ...
    int n_cells[10];       // Number of cells for each type
    int axons[10];         // Binary axon descriptors of interneuron types
    int dendrites[10];     // Binary dendrite descriptors of interneuron types
    double polarities[10]; // Polarities of interneuron types
    // decay = exp(-((dist - beta) / phi)^2)
    double phi[10];        // Scale, in [1, WIDTH)
    double beta[10];          // Center, in [0, WIDTH)
