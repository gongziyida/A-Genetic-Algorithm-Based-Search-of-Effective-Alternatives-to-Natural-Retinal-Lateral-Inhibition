...
//    vdRngUniform(VSL_RNG_METHOD_UNIFORM_STD, STREAM, 1, &rp->decay, 0, WIDTH);
    ...
    if (rp->polarities[0] < 0) rp->polarities[0] = fabs(rp->polarities[0]);

    viRngUniform(VSL_RNG_METHOD_UNIFORM_STD, STREAM, n, rp->n_cells, 1, MAX_CELLS);
    rp->n_cells[0] = MAX_CELLS;

    vdRngUniform(VSL_RNG_METHOD_UNIFORM_STD, STREAM, n, rp->phi, 1, WIDTH / 2);

    vdRngUniform(VSL_RNG_METHOD_UNIFORM_STD, STREAM, n, rp->beta, 0, WIDTH / 2);
...
void process(RetinaParam *rp, double *input) {
    ...
    double o[MAX_CELLS]; // For storing the output of a type of neurons
    ...
    */
        ...
        for (i = 0; i < n; i++){
            ...
            for (j = 0; j < n; j++){
                ...
                for (k = 0; k < nj; k++) { // Sigmoid
                    o[k] = 1 / (1 + exp(-rp->old_states[j * MAX_CELLS + k]));
                }
    ...
    for (i = 0; i < MAX_CELLS; i++){
        if (rp->new_states[i] < min_val) min_val = rp->new_states[i];
        if (rp->new_states[i] > max_val) max_val = rp->new_states[i];
    }
    for (i = 0; i < MAX_CELLS; i++)
        rp->new_states[i] = (rp->new_states[i] - min_val) / (max_val - min_val);
